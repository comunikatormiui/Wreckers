...
public class HostDamageSystem extends EntitySystem {
	...
	private void onWeaponWreckerHitEvent(WeaponWreckerHitEvent e) {
		...
		if (additionalImpulse > 1) {
			Vector2 box2dPos = vec1.set(e.getPoint()).scl(1 / Game.scale);
		...
		//			wjd.localAnchorB.set(e.getWreckerBody().getLocalPoint(new Vector2(e.getPoint()).scl( 1 / Game.box2dScale)));
		//			World world = e.getWeaponBody().getWorld();
		//			world.createJoint(wjd);
		//			System.out.println("WJD: " + new Vector2(e.getPoint()).scl(1 / Game.box2dScale));
