...
public class CollisionSystem extends EntitySystem{
    ...
    private void handleWeaponToPlayer(final Entity weapon, EntityType weaponType, final Entity player, EntityType playerType, Contact contact, ContactImpulse impulse, boolean weaponIsA){
        ...
        float dullPercent = calculateDullness(weaponBody, point, normal); // на сколько процентов данный удар €вл€етс€ пр€мым
        ...
        if (weaponFixtureType == FixtureType.WEAPON_DAMAGE) { // если ударивша€ часть €вл€етс€ дамажущей
            ...
            //final float directDamage = (wc.hitDamage   * (impulseForce / 100)) * dullPercent;  // –ассчитываем пр€мой урон
            //final float sharpDamage  = (wc.sliceDamage * (impulseForce / 100)) * sharpPercent;   // –ассчитываем режущий урон
            //final Vector2 force = new Vector2(normal).scl(playerBody.getMass() * wc.hitImpulse); // —ила дополнительного отталкивани€
        }
