...
public class ClientBase implements Iterable<Client> {
    ...
    private final Array<Listener> listeners = new Array<Listener>();
    private final Array<Ban> bans = new Array<Ban>();
    private volatile int size;

    public ClientBase() {
        processingIterator = new Array.ArrayIterator<Client>(clients);
        sendAllIterator = new Array.ArrayIterator<Client>(clients);
    }
    ...
    public void ban(InetAddress address, int banDuration, TimeUnit timeUnit){
        ...
        if (bannedClient != null){
            bannedClient.getSocket().close(DCAssets.KICK);
            remove(bannedClient);
        }
    }
